...
class Chat
{
    public function sendHeaders($headersText, $newSocket, $host, $port){
        ...
        $strHeadr = "HTTP/1.1 101 Switching Protocols \r\n".
            ...
            "WebSocket-Location: ws://$host:$port/WebSocketChat/server.php\r\n".
    ...
    public function send($message, $clientSocketArray){
        ...
        foreach ($clientSocketArray as $cS){
            @socket_write($cS,$message,$messageLength);
        }
        return true;
    }
    ...
    public function createChatMessage($username, $messageStr){
        $message = $username . "->" . "&" . $messageStr . "<br>";
        $messageArray = [
            'type'=>'chat-box',
            'message'=>$message
        ];
